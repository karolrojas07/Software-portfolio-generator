import Config

# Configure your database
#
# The MIX_TEST_PARTITION environment variable can be used
# to provide built-in test partitioning in CI environment.
# Run `mix help test` for more information.
config :software_portfolio_generator, SoftwarePortfolioGenerator.Repo,
  username: System.get_env("DB_TEST_USERNAME", "postgres"),
  password: System.get_env("DB_TEST_PASSWORD", "postgres"),
  hostname: System.get_env("DB_TEST_HOSTNAME", "localhost"),
  database: System.get_env("DB_TEST_DATABASE", "software_portfolio_generator_test"),
  pool: Ecto.Adapters.SQL.Sandbox,
  pool_size: System.schedulers_online() * 2

# We don't run a server during test. If one is required,
# you can enable the server option below.
config :software_portfolio_generator, SoftwarePortfolioGeneratorWeb.Endpoint,
  http: [ip: {127, 0, 0, 1}, port: 4002],
  secret_key_base: System.get_env("SECRET_KEY_BASE", "LAwl+dihSK32JOJ2x16/cRsvtIkJdt2vwRR8v69t5j/6bKkcu96w0WaBpUWH1/Wa"),
  server: false

# In test we don't send emails
config :software_portfolio_generator, SoftwarePortfolioGenerator.Mailer,
  adapter: Swoosh.Adapters.Test

# Disable swoosh api client as it is only required for production adapters
config :swoosh, :api_client, false

# Print only warnings and errors during test
config :logger, level: :warning

# Initialize plugs at runtime for faster test compilation
config :phoenix, :plug_init_mode, :runtime

# Enable helpful, but potentially expensive runtime checks
config :phoenix_live_view,
  enable_expensive_runtime_checks: true
